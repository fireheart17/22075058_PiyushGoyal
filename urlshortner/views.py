...
def createShortURL(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            s.save()
            return render(request,'urlcreated.html',{'chars':settings.BASE_URL+str(random_chars)})
    else:
...
def fetch_created_urls(request):
    q=ShortURL.objects.all()
    return render(request,'showTables.html',{'url_list':q,'base':str(settings.BASE_URL)})
